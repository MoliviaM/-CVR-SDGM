...
class Dataset4DualEncoding(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # frames_tensor=[20,4096] cap_tensor=[7,] cap_bow=[8098,] bert_feat=[1024,] index=62172 cap_id='video2412#enc#12'  video_id='video2412' vid_tag=[512,]
...
def get_data_loaders(cap_files, visual_feats, tag_path, tag_vocab_path, vocab, bow2vec, batch_size=100, num_workers=0, video2frames=None):
    """
...
def get_train_data_loaders(opt, cap_files, visual_feats, tag_path, tag_vocab_path, vocab, bow2vec, bert_file, batch_size=100, num_workers=0, video2frames=None):
    """
    Returns torch.utils.data.DataLoader for train and validation datasets
    Args:
        cap_files: caption files (dict) keys: [train, val]
...
def get_test_data_loaders(cap_files, visual_feats, tag_path, tag_vocab_path, vocab, bow2vec, batch_size=100, num_workers=0, video2frames = None):
    """
...
def get_vis_data_loader(vis_feat, batch_size=100, num_workers=0, video2frames=None, video_ids=None):
...
def get_txt_data_loader(opt, cap_file, vocab, bow2vec, bert_file, batch_size=100, num_workers=0):
