...
def main():
    ...
    sample_metrics_file = './sample_metrics.txt'
    ...
    if 'checkpoints' in output_dir:
        ...
        output_dir = '/media/ubun/Student/mmm/rivrl/VisualSearch/result'
    else:
    ...
    pred_error_matrix_file = '/media/ubun/Student/mmm/rivrl/VisualSearch/result/log.pth.tar'
    ...
    text_data_loader = data.get_txt_data_loader(options, caption_files['test'], rnn_vocab, bow2vec, options.bert_file,
    ...
    if options.space != 'latent':
        ...
        evaluation.pred_tag(video_tag_probs, video_ids, tag_vocab_path, os.path.join('video'))
        evaluation.pred_tag(cap_tag_probs, caption_ids, tag_vocab_path, os.path.join('text'))
    ...
    if options.space in ['hybrid']:
        ...
        t2v_all_errors_local_tag = evaluation.cal_error_batch(video_local_tag_probs, cap_local_tag_probs,
                                                              options.measure_2)
        ...
        t2v_all_errors = 0.6 * (t2v_all_errors_global + t2v_all_errors_local) + 0.4 * (
                    t2v_all_errors_global_tag + t2v_all_errors_local_tag)
